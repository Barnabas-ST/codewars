...
"1,2"  =>  NULL*/
    ...
    function array(arr){
      //Good luck
        ...
        let strArr = arr.split(',').slice(1,-1)
       
        
        return strArr.length < 1 ? null : strArr.join(' ')
      }
...
function  calculateAge(birthYear, targetYear) {
  ...
  // enter your code here.
    ...
    return age > 0 && age <= 1 ? `You are ${age} year old.` 
        : age > 1 ? `You are ${age} years old.`
        : age >= -1 && age < 0 ? `You will be born in ${age*-1} year.`
        : age < -1 ? `You will be born in ${age*-1} years.`
...
function derive(coefficient,exponent) {
  ...
  let difference = exponent - 1
  
  return `${product}x^${difference}`
}
...
function pushElement(arr){
  var el=1;
   //push el to arr
  const newArr = arr.push(el)
  return arr
}
function popElement(arr){
  //pop an element from arr
  
  const remArr = arr.pop()
  return arr
}
